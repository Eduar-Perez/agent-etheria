...
def construir_orchestrate_code(job_name: str, stages):
    dataset1 = stages[0]["name"]
    dataset2 = stages[1]["name"]
    join = stages[2]["name"]
    salida = stages[3]["name"]
    return f"""Job {job_name}
(
    PxDataSet {dataset1}('RUTA_DATASET_1.ds')
    -> LINK_1
    -> PxJoin {join}
    -> LNK_SALIDA_JOIN
    -> PxDataSet {salida}('RUTA_DATASET_SALIDA.ds')
)"""
...
def agregar_metabag_default(parent_elem):
    metabag = ET.SubElement(parent_elem, "Collection", {"Name": "MetaBag", "Type": "MetaProperty"})
    for name, value in [
...
def agregar_columnas_completas(parent_elem, columns):
    cols = ET.SubElement(parent_elem, "Collection", {"Name": "Columns", "Type": "OutputColumn"})
    for col in columns:
        ...
        if "Derivation" in col:
            ET.SubElement(col_rec, "Property", {"Name": "Derivation"}).text = col["Derivation"]
            ET.SubElement(col_rec, "Property", {"Name": "ParsedDerivation"}).text = col["Derivation"]
            ET.SubElement(col_rec, "Property", {"Name": "SourceColumn"}).text = col["Derivation"]
...
def agregar_output_pins_completo(job, outputs, nombre_a_id):
    for out in outputs:
        ...
        ET.SubElement(rec, "Property", {"Name": "LeftTextPos"}).text = str(out.get("left", 0))
        ET.SubElement(rec, "Property", {"Name": "TopTextPos"}).text = str(out.get("top", 0))
        ...
        cols = ET.SubElement(rec, "Collection", {"Name": "Columns", "Type": "OutputColumn"})
        for col in out.get("columns", []):
            ...
            if "Derivation" in col:
                for key in ["Derivation", "ParsedDerivation", "SourceColumn"]:
                    ET.SubElement(sub, "Property", {"Name": key}).text = col["Derivation"]
            elif col.get("Name"):
                for key in ["Derivation", "ParsedDerivation", "SourceColumn"]:
                    ET.SubElement(sub, "Property", {"Name": key}).text = col["Name"]

        agregar_metabag_default(rec)

        # Evitar duplicados de OutputPins en el Stage padre
        #for stage_rec in job.findall("Record"):
         #   if stage_rec.attrib.get("Identifier") == stage_id:
          #      existing = stage_rec.findall("./Property[@Name='OutputPins']")
           #     if not any(pin_id == p.text for p in existing):
            #        ET.SubElement(stage_rec, "Property", {"Name": "OutputPins"}).text = pin_id
...
def agregar_input_pins_completo(job, inputs, nombre_a_id):
    for inp in inputs:
        ...
        pin_id = f"{stage_id}P{inp['pin_number']}"
        ...
        # Propiedades si es la salida final
        if inp["stage"] == "DS_SALIDA":
            props = ET.SubElement(rec, "Collection", {"Name": "Properties", "Type": "CustomProperty"})
            sub1 = ET.SubElement(props, "SubRecord")
            ET.SubElement(sub1, "Property", {"Name": "Name"}).text = "dataset"
            ET.SubElement(sub1, "Property", {"Name": "Value"}).text = "RUTA_DATASET_SALIDA.ds"
        ...
        if "columns" in inp:
            ...
            for col in inp["columns"]:
                ...
                if "Derivation" in col:
                    for key in ["Derivation", "ParsedDerivation", "SourceColumn"]:
                        ET.SubElement(col_rec, "Property", {"Name": key}).text = col["Derivation"]
        ...
        #for stage_rec in job.findall("Record"):
         #   if stage_rec.attrib.get("Identifier") == stage_id:
          #      existing = stage_rec.findall("./Property[@Name='InputPins']")
           #     if not any(pin_id == p.text for p in existing):
            #        ET.SubElement(stage_rec, "Property", {"Name": "InputPins"}).text = pin_id
...
def agregar_links_desde_definicion(job, link_props):
    ...
    """
    container_view = next(r for r in job.findall("Record") if r.attrib.get("Identifier") == "V0")

    def agregar_propiedad(name, value):
        ET.SubElement(container_view, "Property", {"Name": name}).text = value

    campos_esperados = [
        "LinkNames", "LinkTypes", "LinkSourcePinIDs", "TargetStageIDs",
        "LinkHasMetaDatas", "LinkNamePositionXs", "LinkNamePositionYs",
        "SourceStageEffectiveExecutionModes", "TargetStageEffectiveExecutionModes",
        "SourceStageRuntimeExecutionModes", "TargetStageRuntimeExecutionModes",
        "LinkIsSingleOperatorLookup", "LinkIsSortSequential",
        "LinkSortMode", "LinkPartColMode"
    ]
...
def agregar_stages_reescrito(job, stages, input_pins, output_pins, nombre_a_id):
    for stage in stages:
        ...
        # Propiedades generales del stage
        for k, v in {
            "Name": stage["name"],
        ...
        # OutputPins asignados a este stage
        out_pins = [f"{stage_id}P{out['pin_number']}" for out in output_pins if out["stage"] == stage["name"]]
        if out_pins:
            ET.SubElement(rec, "Property", {"Name": "OutputPins"}).text = "|".join(out_pins)

        # InputPins asignados a este stage
        in_pins = [f"{stage_id}P{inp['pin_number']}" for inp in input_pins if inp["stage"] == stage["name"]]
        if in_pins:
            ET.SubElement(rec, "Property", {"Name": "InputPins"}).text = "|".join(in_pins)

        # Propiedades especiales si es un PxJoin
...
def agregar_root_record(job, data, orchestrate_code):
    ...
    ET.SubElement(rec, "Property", {"Name": "MFProcessMetaDataXMLFileExchangeMethod"}).text = "0"
    ...
    ET.SubElement(rec, "Property", {"Name": "OrchestrateCode", "PreFormatted": "1"}).text = orchestrate_code
...
def agregar_container_view_completo_reescrito(job, data, nombre_a_id):
    ...
    stage_types = [stage["type"] for stage in data["stages"]]

    link_props = data.get("link_props", {})

    ET.SubElement(rec, "Property", {"Name": "StageList"}).text = "|".join(stage_ids)
    ET.SubElement(rec, "Property", {"Name": "StageXPos"}).text = "|".join(link_props.get("StageXPos", ["0"] * len(stage_ids)))
    ET.SubElement(rec, "Property", {"Name": "StageYPos"}).text = "|".join(link_props.get("StageYPos", ["0"] * len(stage_ids)))
    ET.SubElement(rec, "Property", {"Name": "StageXSize"}).text = "|".join(["48"] * len(stage_ids))
    ET.SubElement(rec, "Property", {"Name": "StageYSize"}).text = "|".join(["48"] * len(stage_ids))
    ET.SubElement(rec, "Property", {"Name": "StageNames"}).text = "|".join(stage_names)
    ET.SubElement(rec, "Property", {"Name": "StageTypeIDs"}).text = "|".join(stage_types)
    ET.SubElement(rec, "Property", {"Name": "StageTypes"}).text = "|".join(["CCustomStage"] * len(stage_ids))
    ...
    ET.SubElement(rec, "Property", {"Name": "NextStageID"}).text = str(len(stage_ids) + 1)
    ...
    ET.SubElement(rec, "Property", {"Name": "ContainerViewSizing"}).text = "0078 0078 0536 0366 0000 0001 0000 0000"
...
def interpretar_y_generar_xml(state: dict) -> dict:
    ...
    * Asegúrate de incluir el atributo 'columns' con los metadatos (Name, SqlType, Precision y Derivation si aplica) para **todos los output_pins**, incluyendo los de DATASET_1 y DATASET_2.
    ...
    Responde con un JSON que siga esta estructura exacta:
    
    {{
      "job_name": "JOB_EJ_JOIN_CONFIG",
      "stages": [
        {{
          "name": "DATASET_1",
          "type": "PxDataSet"
      ...
      "link_props": {{
        "StageXPos": ["120", "120", "288", "480"],
        "StageYPos": ["96", "264", "168", "168"],
        "LinkNames": "LINK_1|LNK_2|LNK_SALIDA_JOIN",
        "LinkTypes": "1|1|1",
        "LinkSourcePinIDs": "V0S0P1|V0S1P1|V0S2P3",
        "TargetStageIDs": "V0S2|V0S2|V0S3",
        "LinkHasMetaDatas": "True|True|True",
        "LinkNamePositionXs": "244|231|344",
        "LinkNamePositionYs": "133|244|175",
    ...
    try:
        ...
        agregar_stages_reescrito(job, data["stages"], data["input_pins"], data["output_pins"], nombre_a_id)
...
def actualizar_input_request_desde_archivos(ruta_prompts: str):
    """
    Lee los archivos desde la ruta especificada, busca los archivos que inician por 'TRF_JN_',
    ...
    archivos_ext = [archivo for archivo in os.listdir(ruta_prompts) if archivo.startswith("TRF_JN_")]

    if not archivos_ext:
        print("No se encontraron archivos que inicien con 'TRF_JN_' en la ruta especificada.")
        return
...
def guardar_xml(xml_content, ruta_directorio="./output", nombre_archivo=None):
    ...
    if nombre_archivo is None:
        ...
        nombre_archivo = f"JOB_TRF_JON_AGENTE_{fecha_hora}.xml"
