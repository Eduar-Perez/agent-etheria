...
def parse_snp_sb(root):
    ...
    for obj in root.findall('.//Object'):
        ...
        if cls.endswith('SnpSbStep'):
            ...
            scen_task_no = task_name1 = col_lschema = col_txt = def_lschema = def_txt = ord_trt = None
            for field in obj.findall('Field'):
                name = field.get('name')
                text = field.text.strip() if field.text and field.text.strip().lower() != 'null' else None
                if name == 'ScenTaskNo':
    ...
    for step in details:
        step['subdetails'].sort(key=lambda x: (x['ORD_TRT'] if x['ORD_TRT'] is not None else float('inf'),
...
def detect_and_parse(xml_path):
    ...
    if root.find('.//Object[@class="com.sunopsis.dwg.dbobj.SnpSb"]') is not None:
        ...
        return {'message': 'Unknown XML structure', 'file': os.path.basename(xml_path)}, 'unknown'
